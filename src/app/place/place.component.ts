...
export class PlaceComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    this.map.on('singleclick', (e) => {
      ...
      if (zoom) {
        ...
        self.selectedImage =
          `http://digitiles.itwis-demos.com/tiles/26x${tileCord[1]}x${tileCord[2]}.png?` +
          Math.random();
  ...
  getTileUrl(): string {
    return 'http://digitiles.itwis-demos.com/tiles/{z}x{x}x{y}.png?v=' + this.tileVersion;
  }
  ...
  initWebSocket(): void {
    this.socket = new WebSocket('wss://digitiles.itwis-demos.com:10077');
